...
from torch_geometric.utils import remove_self_loops, add_self_loops, softmax, dropout_adj, \
    is_undirected, negative_sampling, batched_negative_sampling, to_undirected
...
class SuperEdgeGO(MessagePassing):
    # (1280, 512, 1, 0.2, 0.2, 0.5, 1.0, 0.0)

    def __init__(self, in_channels, out_channels, heads=1, concat=True, negative_slope=0.2, dropout=0.2, bias=True,
                 is_super_gat=True, attention_type="basic", super_gat_criterion=None,
                 neg_sample_ratio=0.0, edge_sample_ratio=1.0,         # Pn∈R+  0.3, 0.5, 0.7, 0.9, Pe∈(0, 1] 0.6, 0.8, 1.0
        ...
        super(SuperEdgeGO, self).__init__(aggr='add', node_dim=0, **kwargs)
        ...
        if self.is_super_gat:
            ...
            if self.attention_type == "gat_originated":  # GO gat_originated
    ...
    def message(self, edge_index_i, x_i, x_j, size_i):
        ...
        # # Compute a "fake" attention coefficients that are actually 1/n (i.e., average weights without attention).
        # device = next(self.parameters()).device
        # avg_alpha = torch.ones(x_j.shape[0], x_j.shape[1]).to(device)
        # avg_alpha = softmax(avg_alpha, edge_index_i, num_nodes=size_i)
        # alpha = avg_alpha
        # # II. WITHOUT ATTENTION ---------------
    ...
    def get_supervised_attention_loss(model, criterion=None):
        ...
        loss_list = []
        cache_list = [(m, m.cache) for m in model.modules() if m.__class__.__name__ == SuperEdgeGO.__name__]
    ...
    def mix_supervised_attention_loss_with_pretraining(loss, model, mixing_weight,
        ...
        for m in model.modules():
            if m.__class__.__name__ == SuperEdgeGO.__name__:
        ...
        loss = w1 * loss + w2 * SuperEdgeGO.get_supervised_attention_loss(
            model=model,
            criterion=criterion,
        )
        return loss
